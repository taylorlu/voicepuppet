...
class Pix2PixNet(ModelBuilder):
  ...
  def __init__(self, config_path):
    ...
    self.__params = Pix2PixNet.default_hparams(config_path)
  ...
  def default_hparams(config_path, name='default'):
    ...
    params.add_hparam('l1_weight', 100.0)
    params.add_hparam('gan_weight', 1.0)

    params.training['learning_rate'] = 0.0005
    params.training['beta1'] = 0.5
    params.training['decay_rate'] = 0.99
  ...
  def build_network(self, inputs, targets, trainable=True):
    ...
    with tf.variable_scope("generator"):
      out_channels = int(targets.get_shape()[-1]) + 1
      outputs = create_generator(inputs, out_channels)
      rgb = outputs[:,:,:,:3]
      alpha = (outputs[:,:,:,3:]+1)/2
      alpha = tf.tile(alpha, [1,1,1,3])
      outputs = rgb * alpha + targets * (1 - alpha)
      # outputs = outputs * masks + targets * (1 - masks)
      nodes.update({'Outputs': outputs})
  ...
  def add_cost_function(self, predict_real, predict_fake, targets, outputs, alphas, masks):
    nodes = {}
  ...
  def build_train_op(self, inputs, targets, masks):
    ...
    loss_dict = self.add_cost_function(nodes['Predict_real'], nodes['Predict_fake'], targets, nodes['Outputs'], nodes['Alphas'], nodes['Masks'])
