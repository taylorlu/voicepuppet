...
def write_dataset(params):
  ...
  landmark_name = params.sample_file['landmark_name']
  wav_name = params.sample_file['wav_name']
  bfmcoeff_name = params.sample_file['bfmcoeff_name']
  ...
  with open(train_dataset_path, "w") as train_file:
    with open(eval_dataset_path, "w") as eval_file:
      for root, subdirs, files in os.walk(root_path):
        if not subdirs:
          if (os.path.exists(os.path.join(root, landmark_name)) and
              os.path.exists(os.path.join(root, wav_name)) and
              os.path.exists(os.path.join(root, bfmcoeff_name))):
