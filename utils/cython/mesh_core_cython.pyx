...
cdef extern from "mesh_core.h":
    ...
    void _render_colors_core(
        unsigned char* image, unsigned char *face_mask, float* vertices, int* triangles, 
        float* colors, 
        float* depth_buffer,
        int ntri,
        int h, int w, int c)
...
def render_colors_core(np.ndarray[unsigned char, ndim=1, mode = "c"] image not None, 
                np.ndarray[unsigned char, ndim=1, mode = "c"] face_mask not None, 
                np.ndarray[float, ndim=1, mode = "c"] vertices not None, 
                np.ndarray[int, ndim=1, mode="c"] triangles not None, 
                np.ndarray[float, ndim=1, mode = "c"] colors not None, 
                np.ndarray[float, ndim=1, mode = "c"] depth_buffer not None,
                int ntri,
                int h, int w, int c
                ):   
    _render_colors_core(
        <unsigned char*> np.PyArray_DATA(image),  <unsigned char*> np.PyArray_DATA(face_mask), <float*> np.PyArray_DATA(vertices), <int*> np.PyArray_DATA(triangles),  
