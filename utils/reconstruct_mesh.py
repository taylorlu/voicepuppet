...
def Projection_layer(face_shape, rotation, translation, focal=1015.0,
                     center=112.0):  # we choose the focal length and camera position empirically
...
def Illumination_layer(face_texture, norm, gamma):
  # gamma = np.zeros(gamma.shape, dtype=gamma.dtype)
...
def Reconstruction(coeff, facemodel):
  ...
  return face_shape, face_texture, face_color, face_projection, z_buffer, landmarks_2d, translation


# face reconstruction with coeff and BFM model
def Reconstruction_rotation(coeff, facemodel, angles):
  id_coeff, ex_coeff, tex_coeff, _, gamma, translation = Split_coeff(coeff)
  # compute face shape
  face_shape = Shape_formation(id_coeff, ex_coeff, facemodel)
  # compute vertex texture(albedo)
  face_texture = Texture_formation(tex_coeff, facemodel)
  ...
  # rotation matrix
  face_shape = np.matmul(face_shape, rotation)
  ...
  return face_shape, face_texture, face_color, face_projection, z_buffer, landmarks_2d
